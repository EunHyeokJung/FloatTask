...
- `docs/assets`: 격리된 샘플 데이터로 촬영한 기능별 README 화면. 메인·채팅·완료·Settings 등 실제 네이티브 창을 각각 촬영하고 README에서 나란히 배치해 보조 패널 관계를 보여줍니다. 한국어 README는 `-ko` 이미지의 한국어 프로젝트·Task·대화를 사용하며 앱의 영어 고정 UI는 유지합니다. 채팅은 실제 요청·응답을 사용하며 실제 사용자 데이터나 생성한 UI 목업은 포함하지 않습니다.
